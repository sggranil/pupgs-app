...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"enrollees" | "thesis" | "room">(
    "enrollees"
  );
  ...
  return (
    <div className="h-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-2">
          <div className="bg-white shadow-md rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-bgPrimary">
              {subjectData?.filter(
                (subject) => subject.subject_name === "Pre-Oral Defense"
              ).length || 0}
            </p>
          </div>
          <div className="bg-white shadow-md rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-bgPrimary">
              {subjectData?.filter(
                (subject) => subject.subject_name === "Thesis Proposal"
              ).length || 0}
            </p>
          </div>
          <div className="bg-white shadow-md rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-bgPrimary">
              {subjectData?.filter(
                (subject) => subject.subject_name === "Final Defense"
              ).length || 0}
            </p>
          </div>
          <div className="bg-white shadow-md rounded-lg p-4">
      ...
      <div className="h-0">
        ...
        <Modal
          title="Add Thesis"
          isModalOpen={thesisAttachmentsModal}
          setModalOpen={setThesisAttachmentsModal}>
          <AddThesisModal
            setIsModalOpen={setThesisAttachmentsModal}
            setIsUpdated={setIsThesisUpdated}
          />
        </Modal>
      </div>
