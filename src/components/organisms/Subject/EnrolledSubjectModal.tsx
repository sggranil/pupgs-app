...
const EnrolledSubjectModal: React.FC<EnrolledSubjectProps> = ({
  userId,
  subjectData,
  setIsUpdated,
  setIsModalOpen,
  ...
  const {
    ...
    defaultValues: {
      thesis_id: subjectData?.thesis_id,
  ...
  useEffect(() => {
    ...
    if (subjectData?.thesis_id) {
      setValue("thesis_id", subjectData.thesis_id);
    }
  ...
  async function handleFormSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const textData = {
        ...
        thesis_id: Number(values.thesis_id),
      ...
      <div className="w-full py-4">
        ...
        <div className="mb-4">
          ...
          <select
            ...
            {...register("thesis_id")}
            defaultValue={
              subjectData?.thesis_id ? String(subjectData.thesis_id) : ""
            }>
            <option value="" disabled>
              Select Thesis
            </option>
            {thesisData.map((thesis) => (
              <option key={thesis.id} value={String(thesis.id)}>
                {thesis.thesis_title}
              </option>
            ))}
          </select>
