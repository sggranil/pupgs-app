...
interface FormTwelveProps {
  thesisData: Thesis | null;
  programChair: string | null;
  dean: string | null;
}

const FormTwelve: React.FC<FormTwelveProps> = ({
  thesisData,
  programChair,
  dean,
}) => {
    ...
    <div className="w-full px-16 font-inter bg-white shadow-lg rounded-lg mx-auto text-sm leading-normal flex flex-col">
      <div className="text-center mb-6">
        <h1 className="font-bold text-lg uppercase mt-2">
          Thesis/Dissertation Final Endorsement Form
        </h1>
      </div>
      ...
      <p className="text-justify mb-4">
        Upon review of the Thesis/Dissertation Evaluation Committee, the
        aforementioned thesis/ dissertation is now recommended for Final
        Defense.
      </p>

      <div className="flex items-center mb-8">
        <p className="font-semibold w-1/3">Date of Defense:</p>
        <span className="border-b border-black flex-grow px-2 pb-3">
          {defenseSchedule}
        </span>
      </div>
        ...
        <table className="min-w-full border border-gray-300 table-auto">
          ...
          <tbody className="bg-white">
            ...
            {Array.from({ length: 3 }).map((_, index) => {
              // Assuming up to 3 evaluators for dissertation, 2 for thesis
              ...
              return (
                <tr key={`evaluator-${index}`}>
                  ...
                  <td className="border border-gray-300 px-3 py-2">
                    {panelist?.user.first_name || ""}{" "}
                    {panelist?.user.last_name || ""}
                  </td>
                  <td className="border border-gray-300 px-3 py-2">&nbsp;</td>
                  <td className="border border-gray-300 px-3 py-2">&nbsp;</td>
                </tr>
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center mb-1 ml-4">
          <span className="border border-black w-4 h-4 flex-shrink-0 mr-2 flex items-center justify-center text-xs pb-0.5">
            &nbsp;
          </span>
          <p>Final Defense Manuscript</p>
        </div>
        <div className="flex items-center mb-1 ml-4">
          <span className="border border-black w-4 h-4 flex-shrink-0 mr-2 flex items-center justify-center text-xs pb-0.5">
            &nbsp;
          </span>
          <p>Minutes of Pre-Final Defense</p>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-x-16">
        <div>
          ...
          <p className="text-xs text-center">
            (Program Chairperson/Date of Signing)
          </p>
        </div>
        <div>
          ...
          <p className="text-xs text-center">(Dean / Date of Signing)</p>
        </div>
      </div>
    </div>
  );
