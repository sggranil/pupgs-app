...
const FormEleven: React.FC<FormElevenProps> = ({ thesisData, programChair }) => {
    ...
    return (
        <div className="w-full px-16 font-inter bg-white shadow-lg rounded-lg mx-auto text-sm leading-normal flex flex-col">
            <div className="text-center mb-6">
                <h1 className="font-bold text-lg uppercase mt-2">Thesis/Dissertation Panel on Oral Defense Appointment and Acceptance Form</h1>
            </div>
            ...
            <div className="mb-4">
                ...
                <div className="flex items-center mb-2">
                    ...
                    <span className="border-b border-black flex-grow px-2 pb-3">
                        {thesisData?.defense_date ? new Date(thesisData.defense_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : '____________________'}
                    </span>
                </div>
                <div className="mb-2">
            ...
            <div className="overflow-x-auto mb-8">
                <table className="min-w-full border border-gray-300 table-auto">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="border border-gray-300 px-3 py-2 text-left font-semibold w-1/4">Final Defense Panel</th>
                            <th className="border border-gray-300 px-3 py-2 text-left font-semibold w-1/3">Name</th>
                            <th className="border border-gray-300 px-3 py-2 text-left font-semibold w-[25%]">Signature (Signifying Acceptance of Appointment)</th>
                            <th className="border border-gray-300 px-3 py-2 text-left font-semibold w-[15%]">Date of Signing</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white">
                        {/* Chair */}
                        <tr>
                            <td className="border border-gray-300 px-3 py-2 font-semibold">Chair</td>
                        ...
                        {/* Members */}
                        {Array.from({ length: 4 }).map((_, index) => { // Max 4 members (3 for thesis, 4 for dissertation)
                            const panelist = thesisData?.panelists?.[index]; // Assuming panelists array includes chair and members
                            if (panelist && panelist.user.role !== 'chair') { // Only render if it's a member
                                return (
                                    <tr key={`member-${index}`}>
                                        <td className="border border-gray-300 px-3 py-2 font-semibold">Members ({index + 1})</td>
                                        <td className="border border-gray-300 px-3 py-2">
